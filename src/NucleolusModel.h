/*
  ...
  Coupling between particles i and j is scaled by γ(x_mid) where
      γ(x) = clamp(x / L, 0, 1)
  and x_mid is the x-coordinate of the bond midpoint under the minimum-image
  convention: x_mid = x_i - (x_i - x_j)_min_image / 2.
  The backbone bonds (strong intra-polymer cohesion, value ≈ 1000) are NOT
  scaled – they are always at full strength.
...
public:
    ...
    //! γ(x) – chemical coupling parameter at position x. Returns 1 when gradient is off.
